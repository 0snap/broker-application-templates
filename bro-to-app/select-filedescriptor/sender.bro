...
event Broker::peer_added(endpoint: Broker::EndpointInfo, msg: string) {
	...
	schedule 1sec { greet("delayed hi", 1) };
	schedule 2sec { greet("delayed hi", 2) };
	schedule 3sec { greet("delayed hi", 3) };

	schedule 5sec { greet("bye", 1000) };
...
event bro_init() {
	...
	## whenever the event "greet" is called, it will be published automatically on the topic "demo/select_fd"
    Broker::auto_publish("demo/select_fd", greet);
	print "bro init";
}
